...
/*
 * Possible values for the settings of filter-a-on-v4 and
 * filter-a-on-v6: "no" is NONE, "yes" is FILTER, "break-dnssec"
 * is BREAK_DNSSEC.
 */
...
static cfg_type_t cfg_type_parameters = {
	"filter-a-params", cfg_parse_mapbody, cfg_print_mapbody,
...
check_syntax(cfg_obj_t *fmap, const void *cfg, isc_mem_t *mctx, isc_log_t *lctx,
	...
	if ((f4 != NONE || f6 != NONE) && dns_acl_isnone(acl)) {
		cfg_obj_log(aclobj, lctx, ISC_LOG_WARNING,
			    ...
			    "either filter-a-on-v4 or filter-a-on-v6 "
			    "is enabled");
		result = ISC_R_FAILURE;
	} else if (f4 == NONE && f6 == NONE && !dns_acl_isnone(acl)) {
		cfg_obj_log(aclobj, lctx, ISC_LOG_WARNING,
			    "\"filter-a\" is set but "
			    "neither filter-a-on-v4 or filter-a-on-v6 "
			    "is enabled");
		result = ISC_R_FAILURE;
	}
...
parse_parameters(filter_instance_t *inst, const char *parameters,
	...
	CHECK(parse_filter_a_on(param_obj, "filter-a-on-v4",
				   &inst->v4_a));
	CHECK(parse_filter_a_on(param_obj, "filter-a-on-v6",
				   &inst->v6_a));
 ...
 * the client address family and the settings of filter-a-on-v4 and
 * filter-a-on-v6.
 */
static ns_hookresult_t
filter_prep_response_begin(void *arg, void *cbdata, isc_result_t *resp) {
	...
	*resp = ISC_R_UNSET;
	...
	if (qctx->qtype == dns_rdatatype_a) {
		...
		result = dns_db_findrdataset(
			qctx->db, qctx->node, qctx->version, dns_rdatatype_aaaa, 0,
		 ...
		 * DNS_R_DELEGATION or ISC_R_NOTFOUND means there is no AAAA,
		 * and so As are okay.
		 *
		 ...
		 * that we care more about As than As, and would have
			    ...
			    result == ISC_R_NOTFOUND))
		{
			 ...
			 */
			result = ns_query_recurse(qctx->client, dns_rdatatype_aaaa,
